...
└── assets/
    ...
    │   └── ballroom-hero.jpg
    ├── vendor/
    │   └── ffmpeg/                ← bundled MP3 engine (do not rename its files)
    └── audio/
...
- **MP3 export** uses the single-threaded `ffmpeg.wasm` build loaded from a CDN,
  which needs **no special server headers** — ideal for GitHub Pages. The engine
  (~32 MB) is bundled in your repo under `assets/vendor/ffmpeg/`, so no external
  CDN is used; processing runs on-device and is noticeably **slower on phones**.
